...
def format_top_picks(top_picks: dict[str, pd.DataFrame]) -> str:
    """
    Format the top picks per position.
    """
    lines = ["=" * 60, "FPL ORACLE: TOP PREDICTED PICKS (NEXT 3 GAMEWEEKS)", "=" * 60]
    ...
    for pos, pos_df in top_picks.items():
        ...
        for _, row in pos_df.iterrows():
            ...
            lines.append(f"  • {name:<20} ({team:<3}) | £{cost:<4.1f}m | {pts:.2f} pts")
...
def generate_predictions() -> str:
    ...
    # Log to stderr so stdout carries only the digest (e.g. for piping to a notifier)
    print(f"Loading trained Ridge model from: {model_path}", file=sys.stderr)
    model = joblib.load(model_path)

    with open(config_path, "r") as f:
        feature_cols = json.load(f)["feature_columns"]

    print("Generating predictions using model.predict()...", file=sys.stderr)
